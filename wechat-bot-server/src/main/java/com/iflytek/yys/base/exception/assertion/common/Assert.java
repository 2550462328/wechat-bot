...
import com.iflytek.yys.base.exception.WrapMessageException;
...
public interface Assert {
    ...
    /**
     * <p>断言对象（数组）<code>args<code/>都不能为空,如果对象（数组）<code>args</code>有一个为空，则抛出异常</>
     *
     * @param args 待判断对象（数组）
     */
    default void assertAllNotEmpty(Object... args) {

        if (!ObjectUtil.isAllNotEmpty(args)) {
            throw newException();
        }
    }
     ...
     * @param args 待判断对象（数组）
     */
    default void assertAllNotEmptyWithMsg(String msg, Object... args) {

        if (!ObjectUtil.isAllNotEmpty(args)) {
            throw newExceptionWithMsg(msg);
        }
    }
    ...
    /**
     * <p>断言集合<code>c</code>大小为0。如果集合<code>c</code>大小不为0，则抛出异常
     *
     * @param c 待判断数组
     */
